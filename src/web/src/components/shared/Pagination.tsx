  ...
  return (
    <div className="flex flex-col gap-3 py-4 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
      ...
      <nav aria-label="Pagination" className="flex flex-wrap items-center gap-2">
        <Button type="button" variant="ghost" aria-label="Go to previous page" className="min-h-[44px]" disabled={currentPage <= 1} onClick={() => onPageChange(currentPage - 1)}>
          Previous
        </Button>
        ...
        <Button type="button" variant="ghost" aria-label="Go to next page" className="min-h-[44px]" disabled={currentPage >= totalPages} onClick={() => onPageChange(currentPage + 1)}>
          Next
        </Button>
      </nav>
    </div>
