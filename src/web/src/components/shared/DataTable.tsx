...
export function DataTable<T>({
  ...
  // Optimization: Prevent O(N) iteration over all data rows on every render by memoizing
  // selection state and short-circuiting entirely when the table is not selectable.
  // Measurement: Eliminates `data.map` and `filter` overhead when `selectable` is false, and reduces re-computation when `selectable` is true.
  const { selectableRows, selectedCount, allSelected, partiallySelected } = useMemo(() => {
    if (!selectable) {
      return { selectableRows: [], selectedCount: 0, allSelected: false, partiallySelected: false };
    }
