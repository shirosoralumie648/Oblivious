...
export function McpServersPanel({ api }: McpServersPanelProps) {
  ...
  const [toolStateByServerId, setToolStateByServerId] = useState<
  ...
  useEffect(() => {
    ...
    const loadServers = async () => {
      setError(null);
      try {
        const [loadedLocalServers, loaded] = await Promise.all([
          mcpApi.listLocalServers(),
          mcpApi.listServers(),
        ]);
        if (!cancelled) {
  ...
  const updateToolState = (
    serverId: string,
    updater: (current: ToolState) => ToolState,
  ) => {
  ...
  const executeTool = async (serverId: string) => {
    ...
    try {
      const result = await mcpApi.executeTool(serverId, {
        args,
      ...
      updateToolState(serverId, (current) => ({
        ...current,
        result,
        selectedToolName: trimmedToolName,
      }));
    } catch (caughtError) {
      ...
      <div className="mt-5 space-y-3">
        ...
        {servers.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No remote MCP servers registered.
          </p>
        ) : null}
        {servers.map((server) => {
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  ...
                  {server.lastConnectedAt ? (
                    <p className="mt-1 text-xs text-muted-foreground">
                      Last connected: {server.lastConnectedAt}
                    </p>
                  ) : null}
                  {toolState.diagnostic ? (
                    <p className="mt-2 text-sm font-medium text-primary">
                      Diagnostic: {toolState.diagnostic}
                    </p>
                  ) : null}
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    type="button"
                  >
                    {loadingAction === `connect:${server.id}` ? (
                      <RiLoader4Line
                  ...
                  <button
                    ...
                    type="button"
                  >
                    {loadingAction === `disconnect:${server.id}` ? (
                      <RiLoader4Line
                        className="size-4 animate-spin"
                        aria-hidden="true"
                      />
                  ...
                  <button
                    ...
                    type="button"
                  >
                    {loadingAction === `diagnose:${server.id}` ? (
                      <RiLoader4Line
                        className="size-4 animate-spin"
                        aria-hidden="true"
                      />
                  ...
                  <button
                    ...
                    type="button"
                  >
                    {loadingAction === `delete:${server.id}` ? (
                      <RiLoader4Line
                        className="size-4 animate-spin"
                        aria-hidden="true"
                      />
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)]">
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={
                        loadingAction === `execute:${server.id}` ||
                        (
                          toolName ||
                          toolState.selectedToolName ||
                          ""
                        ).trim() === ""
                      }
                      type="submit"
