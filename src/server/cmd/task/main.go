...
func main() {
	if err := config.RunEntrypoint(context.Background(), releasecontract.EntrypointID("task"), config.EntrypointPreflightOptions{
		...
		log.Fatalf("task preflight failed: %v", err)
	}
}
