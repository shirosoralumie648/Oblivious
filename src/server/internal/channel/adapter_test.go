...
func TestWeChatAdapterTransformsXMLInboundAndOutbound(t *testing.T) {
	...
	message, err := adapter.TransformInbound(json.RawMessage(`<xml>
	...
	if message.ID != "wechat_xml_1" || message.ConversationID != "user_openid" || message.Role != RoleUser {
		t.Fatalf("unexpected XML inbound message: %+v", message)
	}
	if len(message.Content) != 1 || message.Content[0].Type != ContentTypeText || message.Content[0].Text != "hello xml" {
	...
	raw, err := adapter.TransformOutbound(InternalMessage{
		...
		ConversationID: "user_openid",
		Role:           RoleAssistant,
		Content:        []ContentPart{{Type: ContentTypeText, Text: "wechat XML reply"}},
