...
func TestRegisterKnowledgeAliasRoutesDispatchesDocumentUpload(t *testing.T) {
	...
	if recorder.Code != stdhttp.StatusOK && recorder.Code != stdhttp.StatusAccepted {
		t.Fatalf("upload expected 200 or 202, got %d with body %s", recorder.Code, recorder.Body.String())
	}
	if len(store.createdIngestionJobs) != 1 || store.createdIngestionJobs[0].KnowledgeBaseID != "kb_2" || store.createdIngestionJobs[0].Title != "Runbook.txt" {
		t.Fatalf("expected upload kb_2/Runbook.txt, got jobs=%+v", store.createdIngestionJobs)
	}
}
