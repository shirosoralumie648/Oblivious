...
impl="$repo_root/scripts/collect_provider_runtime_config_evidence.py"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/collect-provider-runtime-config-evidence.sh \
  --artifact-id artifact-provider-runtime-config-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/provider-runtime-config-proof.json \
...
Builds a provider-runtime-config artifact body from target provider runtime evidence.
