...
impl="$repo_root/scripts/collect_relay_batch_evidence.py"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/collect-relay-batch-evidence.sh \
  --artifact-id artifact-relay-batch-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/relay-batch-proof.json \
...
Builds a relay-batch-proof artifact body from target Batch lifecycle evidence.
