...
impl="$repo_root/scripts/collect_grpc_smoke_report_evidence.py"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/collect-grpc-smoke-report-evidence.sh \
  --artifact-id artifact-grpc-smoke-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/grpc-smoke.json \
...
Builds a grpc-smoke-report artifact body from target generated-client smoke
JSON produced by scripts/target-grpc-smoke.sh or oblivious-grpc-smoke.
EOF
}
