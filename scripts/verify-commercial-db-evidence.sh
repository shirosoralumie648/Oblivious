...
run_quota_sql_isolation_profile() {
  ...
  quota_store_pattern="^TestSQLStore(UsageLimitSettingsRoundTrip|UserQuotaModeUsesUserScopedBalance|BillingSessionsAreOrganizationScoped|TopupOrderMutationsRequireOrganizationScope|ResolveUsageLimitFallsBackToActiveSubscriptionRequestCap)$"
  quota_http_pattern="^Test(CrossTenantQuotaScopeUsesActiveOrganization|AdminUserQuotaRoutePersistsWithPostgres|BillingCheckoutTopupDoesNotCreditQuotaBeforeWebhook|QuotaTopupEndpointNoLongerCreditsWithoutPayment|AdminBillingRecordsTopupRefundAndAdjustsQuota)$"
