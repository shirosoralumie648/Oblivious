...
impl="$repo_root/scripts/collect_microservice_database_evidence.py"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/collect-microservice-database-evidence.sh \
  --artifact-id artifact-microservice-databases-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/microservice-database-proof.json \
...
Builds a microservice-database-proof artifact body from target service database evidence.
