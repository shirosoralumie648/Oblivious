...
collector="$repo_root/scripts/collect-marketplace-governance-evidence.sh"
...
fail() {
  echo "[collect-marketplace-governance-evidence-fixtures] $*" >&2
  exit 1
}
...
artifact_body="$artifact_dir/artifact-marketplace-governance-20260616.json"
...
bash "$collector" \
  --artifact-id artifact-marketplace-governance-20260616 \
  --commit "$(git -C "$repo_root" rev-parse HEAD)" \
  --run-id target-release-20260616 \
  --recorded-at 2026-06-16T01:00:00Z \
  --proof-file "$governance_proof_file" \
...
echo "[collect-marketplace-governance-evidence-fixtures] generated marketplace governance artifact body"

bad_proof_file="$tmpdir/marketplace-governance-proof-bad.json"
...
if bash "$collector" \
  ...
  --output "$tmpdir/bad-governance-artifact.json" >"$bad_output" 2>&1; then
  cat "$bad_output" >&2
  fail "failed review SLA proof unexpectedly passed"
fi
if ! grep -Fq -- "reviewSLAEnforcement must be pass" "$bad_output"; then
  cat "$bad_output" >&2
  fail "bad governance proof failed without reviewSLAEnforcement diagnostic"
fi
echo "[collect-marketplace-governance-evidence-fixtures] rejected failed review SLA proof"

bad_summary_file="$tmpdir/marketplace-governance-proof-bad-summary.json"
...
if bash "$collector" \
  ...
  --output "$tmpdir/bad-governance-summary-artifact.json" >"$bad_summary_output" 2>&1; then
  cat "$bad_summary_output" >&2
  fail "incomplete governance appeal decision summary unexpectedly passed"
fi
if ! grep -Fq -- "summary.appealDecisions must equal summary.appealQueueItems" "$bad_summary_output"; then
  cat "$bad_summary_output" >&2
  fail "bad governance summary failed without appeal decision diagnostic"
fi
echo "[collect-marketplace-governance-evidence-fixtures] rejected incomplete governance appeal decision summary"
