...
ARTIFACT_KIND = "deployment-log"
PROOF_FIELDS = ["deployValidation", "backupRestore", "migrationReplay"]
...
def fail(message):
    raise SystemExit(f"[collect-deployment-evidence] {message}")
...
def build_artifact(args):
    proof = target_evidence_source.read_proof(args, read_json, fail, PROOF_FIELDS)
...
def main():
    ...
    print(f"[collect-deployment-evidence] wrote {output}")
