...
impl="$repo_root/scripts/collect_relay_realtime_evidence.py"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/collect-relay-realtime-evidence.sh \
  --artifact-id artifact-relay-realtime-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/relay-realtime-proof.json \
...
Builds a relay-realtime-proof artifact body from target Realtime lifecycle evidence.
