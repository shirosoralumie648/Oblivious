...
impl="$repo_root/scripts/collect_marketplace_payout_evidence.py"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/collect-marketplace-payout-evidence.sh \
  --artifact-id artifact-marketplace-payouts-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/marketplace-payout-proof.json \
...
Builds a marketplace-payout-proof artifact body from target payout evidence.
