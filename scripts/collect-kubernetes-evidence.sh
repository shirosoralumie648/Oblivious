...
impl="$repo_root/scripts/collect_kubernetes_evidence.py"
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'EOF'
Usage: bash scripts/collect-kubernetes-evidence.sh \
  --artifact-id artifact-k8s-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/kubernetes-proof.json \
