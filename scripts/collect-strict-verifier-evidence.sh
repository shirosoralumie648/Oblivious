...
impl="$repo_root/scripts/collect_strict_verifier_evidence.py"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/collect-strict-verifier-evidence.sh \
  --artifact-id artifact-strict-verifier-... \
  --commit <git-commit> \
  --run-id <target-release-run-id> \
  --recorded-at <ISO-8601 timestamp> \
  --proof-file /path/outside/git/strict-verifier.json \
