...
| Knowledge base and RAG | Functional logic 3.1-3.4 and 7.6; Fusion design 3.3 | `docs/reports/2026-06-07-knowledge-requirement-audit.md` maps Knowledge/RAG requirements. `vector_only` API/store naming, legacy `"vector"` input normalization, knowledge-base RAG config readback, hybrid SQL retrieval shape, citation metadata scanning, Qdrant collection wiring, and SQL-backed retrieval test case persistence have focused tests. | Partial | Remaining gaps: main ingestion does not yet apply knowledge-base chunking config, `hybrid_rerank` does not call a reranker, incremental/versioned update semantics are incomplete, citation highlight/source-preview behavior is not end-to-end proven, and Qdrant has collection management but not chunk upsert/search. |
