...
| Multi-channel publishing | Functional logic 5.1-5.3; Part 2 3.7 | Publishing channel package, platform adapters, worker, message transformer, logs, routes, and workspace publishing UI exist. Automatic due-retry processing now shares the same channel health threshold as manual sends: consecutive retry successes recover degraded channels to active, consecutive retry failures degrade active channels, and retry-triggered degradation routes through the publishing-channel alert/recovery path. Message logs now have a default 30-day configurable retention policy and a batched expired-log archive/delete store path that preserves retry queues. | Partial | Verify remaining platform-specific formatting edges, full administrator notification delivery matrix, object-storage archival worker, and operator controls. |
