...
| Multi-channel publishing | Functional logic 5.1-5.3; Part 2 3.7 | Publishing channel package, platform adapters, worker, message transformer, logs, routes, and workspace publishing UI exist. Automatic due-retry processing now shares the same channel health threshold as manual sends: consecutive retry successes recover degraded channels to active, and consecutive retry failures degrade active channels. | Partial | Verify platform-specific formatting, failure notification path, logs, and operator controls. |
