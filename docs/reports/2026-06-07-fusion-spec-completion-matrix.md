...
| Knowledge base and RAG | Functional logic 3.1-3.4 and 7.6; Fusion design 3.3 | `docs/reports/2026-06-07-knowledge-requirement-audit.md` maps Knowledge/RAG requirements. `vector_only` API/store naming, legacy `"vector"` input normalization, knowledge-base RAG config readback, hybrid SQL retrieval shape, configured HTTP reranking for `hybrid_rerank`, fixed-size chunking config in main ingestion, citation metadata scanning, Qdrant collection wiring, and SQL-backed retrieval test case persistence have focused tests. | Partial | Remaining gaps: semantic/QA/template service-path chunking needs full coverage, reranker candidate pool/fallback observability needs hardening, incremental/versioned update semantics are incomplete, citation highlight/source-preview behavior is not end-to-end proven, and Qdrant has collection management but not chunk upsert/search. |
