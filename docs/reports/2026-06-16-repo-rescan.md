...
- The target/live manifest gate requires an `artifacts[]` index with unique ids, concrete `kind`/`uri`, ISO-8601 `recordedAt`, optional 64-character hex `sha256`, no dangling non-placeholder `evidenceRef` values, and no unreferenced artifact entries. This proves manifest internal consistency only; it does not download or authenticate the external artifacts.
