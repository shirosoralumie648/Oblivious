...
- Branch: `main`.
- Current local-artifact-uri hardening continuation base: `a84f1c3` (`test(release): reject loopback target urls`).
- Remote parity at local-artifact-uri hardening start: `HEAD == origin/main` (`a84f1c3`).
- Working tree at local-artifact-uri hardening start: clean.
...
- Current progress estimate remains about `99/100`: repository-local evidence is broad, the target manifest verifier is now being tightened around collection/artifact/URI consistency, and final completion is still gated by target/live proof plus a strict no-skip release run.
...
- Target/live release evidence now has a machine-readable manifest verifier. The strict commercial verifier requires `COMMERCIAL_COMPLETION_RUN_TARGET_EVIDENCE=true` for final readiness, and the manifest validator checks exact commit, strict no-skip verifier result, deployment/backup/migration evidence, Kubernetes validation/rollout/failover evidence, live provider checkout/refund/payout/reconciliation evidence through `providers[]`, Agent/Workflow/Task generated-client gRPC smoke evidence through `grpc[]` plus `grpcSmokeReport`, target secret audit, workflow telemetry `successRate >= 0.99` with an ISO-8601 `start/end` window, no skip-like fields, no embedded secret material, no placeholder artifact refs, and an `artifacts[]` index that resolves every non-placeholder `evidenceRef` to a concrete artifact id. `--print-template` now emits a current-commit external manifest skeleton that is intentionally rejected until every `TODO` artifact reference is replaced with concrete target-run evidence.
...
- The current parallel read-only scan also surfaced a verifier stability gap and closed it in this continuation: target evidence manifests now fail closed when `providers[]` or `grpc[]` is missing instead of raising a Ruby collection error during artifact-kind binding.
...
- Target/live evidence validation now also requires the manifest to carry explicit `providers[]` and `grpc[]` collections. Missing collections fail closed with stable diagnostics, not an exception during artifact-kind binding.
...
- Target/live artifact URIs are now part of the same docs gate fixture: the verifier rejects local filesystem paths such as `/tmp/target-release/deploy.log` and `file:///tmp/target-release/deploy.log`, while preserving remote target artifact schemes such as CI, provider, and observability references.
...
- The target/live manifest gate requires concrete target environment `name`, `class`, and `baseUrl` values plus an `artifacts[]` index with unique ids, evidence-family-matching `kind`, remote target artifact `uri` values, secret-free URI query parameters, ISO-8601 `recordedAt`, optional 64-character hex `sha256`, no dangling non-placeholder `evidenceRef` values, and no unreferenced artifact entries. This proves manifest internal consistency only; it does not download or authenticate the external artifacts.
...
- Target workflow telemetry evidence with `successRate >= 0.99` and a concrete ISO-8601 observation window.
...
- Target evidence artifact URI remote-only validation.
  - Evidence: `scripts/verify-target-release-evidence.sh` now rejects artifact URIs that are empty, local absolute filesystem paths, invalid URIs, missing a URI scheme, or `file:` URLs. `scripts/verify-target-release-evidence-fixtures.sh` proves `/tmp/target-release/deploy.log` and `file:///tmp/target-release/deploy.log` are rejected while the filled current-commit manifest still accepts remote CI, provider, and observability artifact URI schemes.
  ...
  - Boundary: this is repository-local manifest hygiene proof. It does not download or authenticate artifact content, collect target logs, or replace the target/live evidence run.
...
- Missing providers/grpc collection stability.
  ...
  - Verification: `bash scripts/verify-target-release-evidence-fixtures.sh`, `bash scripts/verify-quality-gates.sh`, `COREPACK_HOME=/tmp/codex-corepack bash scripts/check.sh docs`, and `git diff --check` cover the verifier, fixture, docs, and release gate.
  - Boundary: this is repository-local verifier stability proof. It does not add live provider rails or deployed gRPC evidence.
- Target workflow telemetry window validation.
  ...
  - Verification: `bash scripts/verify-target-release-evidence-fixtures.sh`, `bash scripts/verify-quality-gates.sh`, `COREPACK_HOME=/tmp/codex-corepack bash scripts/check.sh docs`, and `git diff --check` cover the verifier, fixture, docs, and release gate.
  - Boundary: this is repository-local manifest validation proof. It does not provide deployed workflow telemetry or final no-skip target evidence.
- Target evidence artifact-index consistency.
  - Evidence: `scripts/verify-target-release-evidence.sh` now requires `artifacts[]`, validates artifact `id`, `kind`, `uri`, `recordedAt`, and optional `sha256`, rejects duplicate artifact ids, treats `/path/outside/git/...` and `TODO` values as placeholders, verifies every non-placeholder `evidenceRef` outside `artifacts[]` resolves to an artifact id, and rejects artifact ids that are not referenced by at least one concrete `evidenceRef`. The fixture fills a valid current-commit manifest with nine concrete artifacts and proves the generated template, missing artifact index, dangling strict-verifier ref, duplicate artifact id, placeholder artifact id/kind/uri, invalid timestamp, invalid sha256, and unused artifact id are rejected.
  - Verification: `bash scripts/verify-target-release-evidence-fixtures.sh`, `bash scripts/verify-quality-gates.sh`, `COREPACK_HOME=/tmp/codex-corepack bash scripts/check.sh docs`, and `git diff --check` cover the verifier, fixture, docs, and release gate.
...
- Target evidence artifact URI secret guard.
  - Evidence: `scripts/verify-target-release-evidence.sh` now rejects artifact URI query or fragment parameters whose names look like token, secret, signature, API key, access key, credential, kubeconfig, or private key material. `scripts/verify-target-release-evidence-fixtures.sh` proves a URI like `https://ci.internal/.../deploy.log?token=...` is rejected while the valid filled manifest still accepts ordinary CI/provider/dashboard URI forms.
  - Verification: `bash scripts/verify-target-release-evidence-fixtures.sh`, `bash scripts/verify-quality-gates.sh`, `COREPACK_HOME=/tmp/codex-corepack bash scripts/check.sh docs`, and `git diff --check` cover the verifier, fixture, docs, and release gate.
  - Boundary: this is local secret-hygiene validation for manifest metadata only. It does not authenticate artifact content or replace the target secret audit.
- Target evidence artifact kind binding.
  - Evidence: `scripts/verify-target-release-evidence.sh` now binds `evidenceRef` paths to expected artifact kinds for strict verifier logs, deployment logs, Kubernetes validation, provider live rails, gRPC smoke reports, secret audits, and workflow telemetry. `scripts/verify-target-release-evidence-fixtures.sh` proves swapping the strict verifier ref with a provider artifact is rejected while the valid filled manifest still allows Agent/Workflow/Task to share the same `grpc-smoke-report` artifact.
  - Verification: `bash scripts/verify-target-release-evidence-fixtures.sh`, `bash scripts/verify-quality-gates.sh`, `COREPACK_HOME=/tmp/codex-corepack bash scripts/check.sh docs`, and `git diff --check` cover the verifier, fixture, docs, and release gate.
  - Boundary: this is manifest semantic consistency proof. It does not validate target artifact content or replace the external target run.
- Target environment placeholder rejection.
  - Evidence: `scripts/verify-target-release-evidence.sh` now rejects placeholder `environment.name`, `environment.class`, and `environment.baseUrl` values. `scripts/verify-target-release-evidence-fixtures.sh` proves `TODO-target-base-url` is rejected and the valid fixture uses a concrete non-example target URL.
  - Verification: `bash scripts/verify-target-release-evidence-fixtures.sh`, `bash scripts/verify-quality-gates.sh`, `COREPACK_HOME=/tmp/codex-corepack bash scripts/check.sh docs`, and `git diff --check` cover the verifier, fixture, docs, and release gate.
  - Boundary: this proves the manifest identifies a concrete target environment in metadata. It does not prove the target URL is reachable.
