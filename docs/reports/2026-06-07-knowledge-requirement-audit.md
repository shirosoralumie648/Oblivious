...
| Chunk visualization/editing is available end to end. | Partial | Workspace UI has retrieval result and chunk actions, and server chunk update/list paths exist. SQL-backed chunk listing and single-chunk updates are now tenant-scoped and tested; single-chunk edits re-embed the updated content and upsert exactly that chunk back to the configured Qdrant vector store with tenant scope, document version, source URL, and page metadata preserved. Backend split/merge APIs now support `POST /chunks/{chunkId}/split` and `POST /chunks/{chunkId}/merge`, atomically reindex SQL chunk rows, delete stale document points from Qdrant, and upsert the re-embedded current chunk list. Covered by `TestSQLStoreListKnowledgeDocumentChunksReturnsTenantScopedChunkViews`, `TestSQLStoreUpdateKnowledgeDocumentChunkUpdatesContentHashAndReturnsChunk`, `TestSQLStoreSplitKnowledgeDocumentChunkReindexesFollowingChunks`, `TestSQLStoreMergeKnowledgeDocumentChunksCombinesNeighborAndReindexes`, `TestUpdateDocumentChunkReindexesEditedChunkInQdrantVectorStore`, `TestSplitDocumentChunkReindexesDocumentChunksInQdrantVectorStore`, `TestKnowledgeHandlerSplitDocumentChunkReturnsReindexedChunks`, `TestKnowledgeHandlerMergeDocumentChunksReturnsReindexedChunks`, and `TestKnowledgeAliasRoutesDispatchChunkSplitAndMerge`. This audit still does not prove frontend split/merge controls or colored chunk overlays. |
...
The current Knowledge/RAG slices close the highest-priority API/store contract gaps for retrieval mode naming, knowledge-base RAG config readback, SQL-backed retrieval test case persistence, SQL-backed chunk listing/update/split/merge operations, fixed/semantic/QA/template chunking config in the main document create/update ingestion path, configured HTTP reranking for `hybrid_rerank`, reranker outage fallback metrics, incremental chunk-hash update semantics, versioned chunk coexistence for update writes, single-chunk edit vector-store consistency, split/merge document vector-store reindexing, citation highlight/source metadata in SQL/API/UI surfaces, Qdrant chunk upsert/search adapter behavior, Qdrant-backed `vector_only` retrieval, Qdrant-backed vector candidates for configured `hybrid`/`hybrid_rerank` retrieval while preserving SQL keyword fusion, and knowledge-base `rerankTopK` candidate-pool expansion before final result truncation. The remaining high-value work is:

1. Prove or complete frontend chunk split/merge controls, colored chunk overlays, full document-version history, and curated retrieval quality benchmark coverage.
