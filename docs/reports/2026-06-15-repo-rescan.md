...
- This pass closes the newly identified local DB evidence-profile opportunity by adding `core-sql-persistence`, closes the MCP Servers built-browser contract candidate, closes the Workflow advanced-controls browser workflow candidate, and closes the Console Notifications browser contract candidate. The obvious local follow-ups from the read-only frontend/API scans are now exhausted; release-boundary drift remains target-bound: a new completion claim still needs the surrounding full Go/test/database/diff commands, deployment proof, Kubernetes proof, backup/restore, and strict no-skip verifier evidence recorded separately.
