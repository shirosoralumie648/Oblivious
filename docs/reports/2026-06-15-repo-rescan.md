...
- The fresh read-only backend/deployment rescan found three local evidence-profile opportunities that are not target-only: Marketplace recommended PostgreSQL search, Billing checkout/top-up HTTP route PostgreSQL proof, and Marketplace template create/list/detail/install PostgreSQL proof. It also found release-boundary drift: `scripts/verify-commercial-completion.sh` does not currently run dependency security or Kubernetes validation even though the fusion evidence pack requires those final commands separately.
...
- It does not yet expose dedicated no-skip profiles for Marketplace recommended PostgreSQL search, Billing checkout/top-up HTTP routes, or Marketplace template create/list/detail/install routes, even though existing skipped-without-DB tests make those good local follow-up slices.
...
- `go test ./internal/marketplace -run 'Test(SearchAgentsRecommended|AddRecommendationMetadata|BuildOrderByRecommended)' -count=1 -v` passed for DB-free recommendation ordering and explanation tests; the DB-backed `SearchAgentsRecommended*` tests skipped in the direct command because `TEST_DATABASE_URL` was not set, so this direct command is not DB proof.
...
1. Local DB evidence: add no-skip `verify-commercial-db-evidence.sh` profiles for Marketplace recommended PostgreSQL search, Billing checkout/top-up HTTP routes, and Marketplace template create/list/detail/install routes.
