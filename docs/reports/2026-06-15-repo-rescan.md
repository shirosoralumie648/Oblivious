...
- Branch: `main`; this follow-up rescan starts from `6757a81 test(db): add core sql persistence evidence profile` and includes the current MCP Servers, Workflow advanced-controls, and Console Notifications built-browser evidence slices.
...
- `src/web/e2e/console-notifications.spec.ts` now supplies built-app Playwright proof for `/console/notifications`: active Console navigation, notification list rendering, server unread-count rendering, single mark-read, mark-all-read, delete, mutation-driven list/count refetches, and severity/category/message display.
...
- This pass closes the newly identified local DB evidence-profile opportunity by adding `core-sql-persistence`, closes the MCP Servers built-browser contract candidate, closes the Workflow advanced-controls browser workflow candidate, and closes the Console Notifications browser contract candidate. The obvious local follow-ups from the read-only frontend/API scans are now exhausted; release-boundary drift remains target-bound: the strict verifier now absorbs the local full Go/test/database/diff gates, but a new completion claim still needs no-skip strict-verifier evidence with deployment proof, Kubernetes proof, backup/restore, and target/live attachments recorded.
- After the Console Notifications browser slice, the next meaningful proof target is environment-bound release validation rather than another first-order local browser/API candidate from this scan.
...
- Test inventory after this slice:
  ...
  - Web Playwright specs: 16 specs
  - Web E2E fixture files: 16 files
...
- The current local DB evidence-profile opportunity is now represented by `core-sql-persistence`; the obvious local frontend/API follow-up candidates from this scan are now covered by MCP Servers, Workflow advanced controls, and Console Notifications browser proof.
...
- `pnpm --dir src/web exec playwright test e2e/console-notifications.spec.ts --project=chromium` passed with `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH=/usr/bin/google-chrome`. It ran one browser test covering `/console/notifications` initial list/count rendering, single mark-read, mark-all-read, delete, and post-mutation list/count refresh behavior.
- `pnpm --dir src/web exec vitest run src/features/notifications/notificationsApi.test.ts src/routes/console/NotificationsPage.test.tsx src/app/router.test.tsx src/app/router.route-surface.test.tsx` passed with 4 files and 110 tests.
...
- Console Notifications now has a built-app Playwright proof in the Console shell. The fixture fails closed if list query parameters, unread-count refreshes, mark-read, mark-all-read, delete, or mutation body contracts drift from the `/api/v1/app/notifications` contract.
...
- The release verifier boundary now absorbs the local full-release gates from the fusion evidence pack: `scripts/verify-commercial-completion.sh` runs docs, Relay security, dependency security, frontend TypeScript, the full server Go suite, focused frontend/browser checks, DB profiles, the serial `TEST_DATABASE_URL` full server Go suite, deploy validation, target-gated Kubernetes validation, backup/restore, and `git diff --check`. A fresh release claim still needs that strict path to pass with no environment skips plus target attachments for Kubernetes, live provider rails, live moderation/notification operations, platform failover, deployed gRPC/client compatibility, and other rows that remain target-bound.
- Kubernetes validation remains target-bound. The repository-local Secret/ConfigMap contract is now guarded statically, but the release smoke still requires a reachable cluster, an external secret file, and the current `k8s-validate.sh` release allowlist before any target Kubernetes readiness claim.

## Recommended Next Slices

1. Target environment: rerun strict release validation with deploy, Kubernetes, backup/restore, failover, live provider/payment/notification rails, target secret audits, and deployed gRPC/client compatibility before any final completion claim.
