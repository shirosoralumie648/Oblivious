...
- Branch: `main`; this follow-up rescan starts from `6757a81 test(db): add core sql persistence evidence profile` and includes the current MCP Servers built-browser evidence slice.
...
- This pass closes the newly identified local DB evidence-profile opportunity by adding `core-sql-persistence` and closes the MCP Servers built-browser contract candidate. Remaining local follow-ups are now mainly Workflow advanced-controls and Console Notifications browser/API contract proof, while release-boundary drift remains target-bound: a new completion claim still needs the surrounding full Go/test/database/diff commands, deployment proof, Kubernetes proof, backup/restore, and strict no-skip verifier evidence recorded separately.
- After the MCP browser slice, the next local proof candidates from the read-only frontend/API scans are Workflow advanced-controls browser workflow first and Console Notifications browser contract second.
...
- The current local DB evidence-profile opportunity is now represented by `core-sql-persistence`; remaining local follow-ups from the fresh scan are Workflow advanced controls and Console Notifications browser/API contract proof.
...
- The freshest local follow-up order from the read-only frontend/API scans is Workflow advanced-controls browser workflow, then Console Notifications browser contract. These are local evidence-depth opportunities, not blockers for this MCP/browser proof slice.
...
1. Workflow advanced-controls browser workflow: add built-browser proof for version history, rollback, branch create/publish/merge, resource-check, and paused failure decision payloads.
2. Console Notifications browser contract: add built-browser proof for unread-count, mark-all-read, single mark-read, and delete UI/API behavior.
