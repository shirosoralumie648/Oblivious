...
- The fresh read-only backend/deployment rescan now has one remaining local evidence-profile opportunity that is not target-only: Billing checkout/top-up HTTP route PostgreSQL proof. It also found release-boundary drift: `scripts/verify-commercial-completion.sh` does not currently run dependency security or Kubernetes validation even though the fusion evidence pack requires those final commands separately.
...
- It does not yet expose a dedicated no-skip profile for Billing checkout/top-up HTTP routes, even though existing skipped-without-DB tests make that a good local follow-up slice.
...
1. Local DB evidence: add a no-skip `verify-commercial-db-evidence.sh` profile for Billing checkout/top-up HTTP routes.
