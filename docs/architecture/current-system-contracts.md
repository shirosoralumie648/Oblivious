...
| Dependency security audit | `bash scripts/check.sh security` | Runs `pnpm audit` against `https://registry.npmjs.org` and `govulncheck` with `OBLIVIOUS_SECURITY_GOTOOLCHAIN` defaulting to `go1.26.4` |
