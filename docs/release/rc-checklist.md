...
| Observability artifacts | `rg -n "RelayOutage|QuotaSettlementFailure|StripeWebhookFailure|MigrationFailure|HighProviderErrorRate|TenantIsolationIncident|OPS-04|OPS-05" deploy/observability docs/release/observability-slos.md docs/release/commercial-gates.md docs/release/rc-checklist.md` | none | Alert-rule, dashboard, and SLO references plus explicit no-final-readiness boundary |
...
- For OPS-04/OPS-05 observability evidence, review [`observability-slos.md`](observability-slos.md), `deploy/observability/prometheus-alerts.yaml`, and `deploy/observability/grafana-dashboard.json`. These artifacts prove repository-local metrics, alert, dashboard, and SLO coverage; external Prometheus/Grafana/OTel/error-tracking deployment remains environment-specific evidence.
