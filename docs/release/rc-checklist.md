...
| Target live evidence manifest | `bash scripts/verify-target-release-evidence.sh --print-template > /path/outside/git/target-release-evidence.json`, then `OBLIVIOUS_TARGET_EVIDENCE_FILE=/path/outside/git/target-release-evidence.json bash scripts/verify-target-release-evidence.sh` | External JSON manifest outside git; no secret values; exact current commit; strict verifier result with no skips; live provider checkout/refund/payout/reconciliation evidence; Agent/Workflow/Task gRPC generated-client smoke; target secret audit; workflow telemetry `successRate >= 0.99`; concrete artifact refs replacing every `TODO` placeholder | Target evidence validation output plus referenced release logs/artifacts |
