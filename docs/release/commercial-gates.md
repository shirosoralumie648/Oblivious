...
| Relay Authority Gate | Production cannot call upstream LLM providers outside Relay; every `/v1/*` endpoint is classified as supported, internal/admin-only, or disabled; unsupported endpoints fail closed; supported endpoints require tenant identity, rate-limit policy, and audit semantics. | Phase 13 completed route classification and production fail-closed behavior. Phase 14 adds `scripts/verify-relay-security.sh`, CI relay-security coverage, trusted internal identity enforcement, supported-route rate-limit policy, and route-decision audit events. Settlement/refund and closeout evidence remain required before this gate is complete. | v05 |
