...
| Frontend shell and pages | Real app-router regression tests, Playwright browser journeys including `src/web/e2e/agent-planning.spec.ts` and `src/web/e2e/chat-solo.spec.ts`, page-focused Vitest suites, matrix row 28 | Route-level shell proof plus page-specific workflow/browser tests, `pnpm --dir src/web exec tsc --noEmit`, targeted Vitest, and targeted Playwright for changed routes |
