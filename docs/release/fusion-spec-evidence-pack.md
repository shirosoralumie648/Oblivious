...
| Agent system | Agent service/store/runtime tests, structured plan-step `description`/`dependsOn` coverage, `scripts/verify-commercial-db-evidence.sh agent-runtime-memory`, Agent route and UI tests, `src/web/e2e/agent-planning.spec.ts`, `src/server/pkg/agent/grpc_server_test.go`, matrix row 24 | ReAct and planning runtime tests, approval/resume tests, memory policy tests, structured plan-step dependency proof, DB-backed persistence proof under `TEST_DATABASE_URL` or disposable pgvector PostgreSQL, gRPC service-boundary tests for changed surfaces, and route/UI/browser tests for changed surfaces |
