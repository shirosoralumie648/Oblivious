...
| Agent system | Agent service/store/runtime tests, structured plan-step `description`/`dependsOn` coverage, `scripts/verify-commercial-db-evidence.sh agent-runtime-memory`, Agent route and UI tests, `src/web/e2e/agent-planning.spec.ts`, matrix row 24 | ReAct and planning runtime tests, approval/resume tests, memory policy tests, structured plan-step dependency proof, DB-backed persistence proof under `TEST_DATABASE_URL` or disposable pgvector PostgreSQL, and route/UI/browser tests for changed surfaces |
...
| Frontend shell and pages | Real app-router regression tests, Playwright browser journeys, page-focused Vitest suites, matrix row 28 | Route-level shell proof plus page-specific workflow/browser tests, `pnpm --dir src/web exec tsc --noEmit`, targeted Vitest, and targeted Playwright for changed routes |
