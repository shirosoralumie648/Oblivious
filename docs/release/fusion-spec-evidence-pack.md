...
| Database-backed tenant isolation | Broader `TEST_DATABASE_URL` integration tests in CI or target release evidence | Focused disposable PostgreSQL app-stateful, auth-security persistence, tenant-membership lifecycle, and tenant-cross-surface evidence covers selected auth/session/rate-limit/ownership/member and core app-surface SQL paths, but DB-free route guards do not prove all SQL persistence isolation |
