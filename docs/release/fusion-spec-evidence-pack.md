...
| Security and tenant isolation | Security row 33, auth/CSRF route-surface tests, tenant isolation tests, secret-redaction contracts, `scripts/verify-commercial-db-evidence.sh tenant-membership-lifecycle`, `scripts/verify-commercial-db-evidence.sh tenant-cross-surface` | DB-free guards where valid, DB-backed tenant-isolation proof where persistence is involved, and dependency scans |
