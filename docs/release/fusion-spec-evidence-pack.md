...
| API contract | `docs/api/openapi.yaml`, `scripts/verify-openapi-contract.sh`, route-surface tests, `src/web/e2e/console-access.spec.ts` fixture payload checks, `src/web/e2e/admin-usage-logs.spec.ts` fixture query checks | Contract gate plus focused route tests for every changed request/response family, including Console-only usage schemas that omit internal provider/channel route fields, browser payload checks for high-value Console mutations, and Admin usage-log/analytics query propagation checks |
