...
| Billing and monetization | Admin/Billing/Relay quota/payment tests, Console API token usage sanitization tests, OpenAPI contract gates, `scripts/verify-commercial-db-evidence.sh`, `scripts/verify-commercial-db-evidence.sh quota-sql-isolation`, `src/web/e2e/console-billing.spec.ts`, matrix row 26 | DB-free handler proof where valid, DB-backed lifecycle and Quota SQL tenant-isolation proof under `TEST_DATABASE_URL` or disposable pgvector PostgreSQL, user-visible Console usage proof without provider/channel route leakage, browser proof for Console subscription checkout payload propagation, and provider proof for external rails |
...
| Frontend shell and pages | Real app-router regression tests, Playwright browser journeys including `src/web/e2e/agent-planning.spec.ts`, `src/web/e2e/chat-solo.spec.ts`, `src/web/e2e/workflows.spec.ts`, and `src/web/e2e/console-billing.spec.ts`, page-focused Vitest suites, matrix row 28 | Route-level shell proof plus page-specific workflow/browser tests, `pnpm --dir src/web exec tsc --noEmit`, targeted Vitest, and targeted Playwright for changed routes |
