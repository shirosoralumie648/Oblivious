...
| Billing and monetization | Admin/Billing/Relay quota/payment tests, Console API token usage sanitization tests, OpenAPI contract gates, `scripts/verify-commercial-db-evidence.sh`, `scripts/verify-commercial-db-evidence.sh quota-sql-isolation`, matrix row 26 | DB-free handler proof where valid, DB-backed lifecycle and Quota SQL tenant-isolation proof under `TEST_DATABASE_URL` or disposable pgvector PostgreSQL, user-visible Console usage proof without provider/channel route leakage, and provider proof for external rails |
