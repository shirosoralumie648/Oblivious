...
| Workflow engine | `docs/reports/2026-06-07-workflow-requirement-audit.md`, `docs/reports/2026-06-08-workflow-success-rate-evidence.md`, `scripts/verify-workflow-success-rate-evidence.sh`, `scripts/verify-commercial-db-evidence.sh scheduled-task-runtime`, `src/web/e2e/workflows.spec.ts` | Repository load evidence, scheduled-task DB runtime proof, workflow browser proof, plus target deployment telemetry for external success-rate claims |
...
| Security and tenant isolation | Security row 33, auth/CSRF route-surface tests, tenant isolation tests, secret-redaction contracts, `scripts/verify-commercial-db-evidence.sh tenant-membership-lifecycle`, `scripts/verify-commercial-db-evidence.sh tenant-cross-surface`, `scripts/verify-commercial-db-evidence.sh secret-response-safety` | DB-free guards where valid, DB-backed tenant-isolation and secret-response proof where persistence is involved, and dependency scans |
...
| Provider/channel secret responses | SQL-backed HTTP redaction proof for remaining provider/channel route families | Admin Observability alert-provider SQL-backed responses are covered; Publishing channel and Admin Relay channel SQL-backed response proof remains open |
